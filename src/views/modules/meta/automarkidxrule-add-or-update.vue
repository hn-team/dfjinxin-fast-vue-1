<template>
  <el-dialog
    ...
    <el-form-item label="标签ID" prop="labelid">
      <el-input v-model="dataForm.labelid" placeholder="标签ID"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.ruleid) {
            this.$http({
              url: this.$http.adornUrl(`/meta/automarkidxrule/info/${this.dataForm.ruleid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.ruleDesc = data.automarkidxrule.ruleDesc
                this.dataForm.labelid = data.automarkidxrule.labelid
                this.dataForm.markIdxObjType = data.automarkidxrule.markIdxObjType
                this.dataForm.objMatchStgy = data.automarkidxrule.objMatchStgy
                this.dataForm.ruleExprs = data.automarkidxrule.ruleExprs
                this.dataForm.tnmtid = data.automarkidxrule.tnmtid
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/meta/automarkidxrule/${!this.dataForm.ruleid ? 'save' : 'update'}`),
              method: 'post',
